...
char* CSVFile::parseTreeLine(char* p, Tree* pTree){
    if ('\0' == *p) return p;
    ...
    while (true){
        ...
        switch (nItem){
            case 1:
                treeNode->m_treeID = std::string(item);
                break;
            case 2:
                treeNode->m_layerIndex = atoi(item);
                break;
            case 3:
                treeNode->m_gene = std::string(item);
                break;
            case 4:
                //vafNode.m_chromosome = std::string(item);
                break;
            ...
            case 12:
                //vafNode.m_altFreq = roundf((float)atof(item),2);
                //binNode.m_altFreq = roundf((float)atof(item),2);
                treeNode->m_nodeAlt = round((float)atof(item)*100);
                pTree->addChildUsingTreeID(treeNode);
                break;
            default:
                cout<<"There is an error in Parsing Tree line. item:"<<item<<endl;
        }
...
int CSVFile::readTreeFile(std::string const &  filename, Tree* pTree){
	m_inputFilename = filename;
	int fileType = determineFileType();
	if (TREE11 != fileType){
		printf("   Error: %s is not a correct tree file.\n", filename.c_str());
		return -1;
	}
	m_pInputFile = fopen(m_inputFilename.c_str(), "rb");
	if (NULL == m_pInputFile) {
		printf("   Sorry. Can not open file %s. If it is open, please close it and retry.\n", filename.c_str());
		return -11;
	}
