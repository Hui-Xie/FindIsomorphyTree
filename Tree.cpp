...
void Tree::addChildUsingTreeID(TreeNode* treeNode){
    if (NULL == treeNode || treeNode->m_treeID.empty() ) return;
    if (0 == treeNode->m_treeID.compare("1")){
        setRoot(treeNode);
    }
    else {
        TreeNode* parent = m_root;
        int pathSize = treeNode->m_treeID.length();
        int i;
        char tag;
        for (i=1; i<pathSize-1;i++){
            tag = treeNode->m_treeID.at(i);
            if ( '0' == tag) parent = parent->m_leftChild;
            else if ('1' == tag) parent = parent->m_rightChild;
            else {
                printf("Error: treeID parse errs in %s in seeking parent.\n",treeNode->m_treeID.c_str() );
            }
        }
        tag = treeNode->m_treeID.at(i);
        if ( '0' == tag) parent->m_leftChild = treeNode;
        else if ('1' == tag) parent->m_rightChild = treeNode;
        else {
            printf("Error: treeID parse errs in %s in new tree node.\n",treeNode->m_treeID.c_str() );
        }
    }
}
